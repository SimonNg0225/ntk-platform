...
test.describe('行銷與付費入口', () => {
  ...
  test('定價頁顯示方案；未設 Stripe 時顯示「即將推出」', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: '一位老師都用得起' })).toBeVisible()
    await expect(page.getByText('免費', { exact: true }).first()).toBeVisible()
    await expect(page.getByText('Pro', { exact: true }).first()).toBeVisible()
    await expect(page.getByText('HK$78 / 月')).toBeVisible()
    // 未配置 Stripe 嘅提示
    await expect(page.getByText(/收費功能正準備開放/)).toBeVisible()
  })
  ...
  test('付費流程入口：未設定 Stripe 撳「升級 Pro」彈出「即將推出」', async ({
    ...
    await expect(page.getByText('收費功能正準備開放，敬請期待。')).toBeVisible()
  })
