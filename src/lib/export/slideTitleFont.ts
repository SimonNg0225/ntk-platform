...
//  ------------------------------------------------------------
//  嵌入字體跨平台不可靠（Mac/Keynote/Google Slides 會走樣），所以改成：
//  瀏覽器用招牌 webfont 在 Canvas 畫標題 → PNG → 當圖擺落封面。
//  係圖片，所以哪個 app 開都一致；正文維持原生可編輯。
//  字體不打包：用先至從 CDN（jsdelivr）載 fontsource CSS（unicode-range
//  分割，瀏覽器只取得用到嗰幾隻字 ~50KB），deploy 零字體重量。
//  全程 browser-guarded + try/catch：任何失敗回 null → cover 使用原生標題。
// ============================================================

import type { CoverTitle } from './types'
export type { CoverTitle }

// CDN（jsdelivr）fontsource CSS — 用先至載，瀏覽器按 unicode-range 只取得用到嗰幾隻字
...
/** pack → 招牌標題字體（family + CDN CSS + latinOnly）。只列有招牌字體的 pack。 */
...
/** 哪個 pack 有招牌字體（UI 決定顯不顯「高擬真」toggle 用） */
...
/**
 * 把 text 用 packId 的招牌字體 render 成 PNG。
