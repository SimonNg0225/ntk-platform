...
//  守門（isAIConfigured / 未登入）留給呼叫端（Inbox.tsx）。
// ============================================================
...
const SYSTEM = `你是一位 GTD（Getting Things Done）整理助手，服務一位香港中學老師兼自學者。
使用者會記下一批「快速擷取」的碎片想法（中英夾雜、口語）。你要替每一條判斷最適合的去向分類：
- task：要去做的行動（批改、購買物品、跟進、提交…）
- note：知識重點、想法、靈感，值得留低參考但不是行動
- event：有特定日期／時間的安排（會議、約會、課堂）
- question：一條想搞清楚／想問 AI／想研究的問題
- countdown：有死線／考試／限期，想倒數提自己
- reference：純連結、書籍、資料，只想 bookmark
只准用以上 6 個英文 key。回應「只准」係一個 JSON 陣列，每個元素 {"i": <原本索引>, "kind": "<key>", "why": "<10 字內極短中文理由>"}，不要有任何其他文字或 markdown。`
 ...
 * 失敗（parse 不到）會 throw 帶友善中文 message。
 */
