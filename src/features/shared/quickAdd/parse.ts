...
//                              解相對日期同分類規則，只回 JSON）。
//    2) toDraft             — **純函數**，容錯 + 正規化 AI 回的 raw
//                              物件（最終資料以呢層為準，不信 AI 格式）。
//    3) parseQuickAdd       — 薄包裝：complete → extractJsonObject →
//                              toDraft；AI / 解析失敗一律回 null。
//  不 import React；正規化邏輯全部可單元測試（見 parse.test.ts）。
// ============================================================
 ...
 * 對應 data/types 的 RecurrenceRule，但只暴露最常見的 daily / weekly：
 *  · freq      —— 'daily' | 'weekly'
 *  · interval  —— 每 N 日/週（正整數，預設 1）
 *  · byWeekday —— 每週重複時指定星期幾（0=日…6=六）；daily 不需要
...
/**
 * 組裝給 Gemini 的 prompt：要他只回**單一 JSON 物件**。
 * 附 today（= todayStr()）同 weekday，令他解到相對日期（明天 /
...
/** 校驗並正規化成 YYYY-MM-DD；不正確（含 AI 吐 "null"）回 undefined。 */
...
/** 校驗並正規化成 HH:mm（24 小時制）；不正確回 undefined。 */
...
/**
 * 正規化 AI 回的 recurrence raw（**不信格式**；不合法整體 drop → undefined）：
 *  · 必須係物件；null / 非物件 / 陣列 → undefined。
 *  · freq 限白名單 {daily, weekly}（大細階不敏感）；不正確 → undefined（整體 drop）。
 *  · interval 正規化成正整數，預設 1（非數字 / ≤0 / 非整數 → 1）。
 *  · byWeekday 只在 weekly 保留：限 0..6 整數、去重、升序；過濾後全空 → 不帶呢欄。
 *    daily 一律不帶 byWeekday。
 */
...
/**
 * 將 AI 回的 raw 物件容錯解析成 ParsedDraft（**純函數，不 call AI**）。
 *  · raw 不是物件 → null
 *  · kind 不在白名單 → 預設 'task'
 *  · title 必須 trim 後非空，否則 → null
 *  · date 正規化成 YYYY-MM-DD（不正確 → undefined）
 *  · time / endTime 正規化成 HH:mm（不正確 → undefined）
 *  · category 只接受合法值（不正確 → undefined）
 *  · recurrence 只在 kind=event 正規化（normRecurrence；不合法 → 不帶）
 *  · event 缺 time 仍可（allDay 由寫入層按 !time 決定，不在這裡）
 ...
 * AI 出錯 / 解不到合法陣列 / 全部 title 空 → 回 []（呼叫端退手動預覽）。
 */
