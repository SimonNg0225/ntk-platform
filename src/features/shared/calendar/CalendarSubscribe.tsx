  ...
  return (
    <header className="-mx-5 -mt-5 mb-5 px-5 pt-5 sm:-mx-6 sm:-mt-6 sm:px-6 sm:pt-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <IconButton
          label={t('calSub.close', { defaultValue: '關閉' })}
          onClick={onClose}
...
export default function CalendarSubscribe({ onClose }: { onClose: () => void }) {
  ...
  const copyLink = useCallback(
    (text: string) => {
      const ok = () => toast.success(t('calSub.copied', { defaultValue: '已複製連結' }))
      const fail = () =>
        toast.error(
          t('calSub.copyFailed', { defaultValue: '複製失敗，請長按連結手動複製' }),
        )
      try {
  ...
  async function handleRotate() {
    ...
    if (!ok) return
    rotateToken()
    toast.success(t('calSub.regenDone', { defaultValue: '已產生新連結，舊連結已失效' }))
  }
  ...
  return (
    ...
    <Modal open onClose={onClose} size="md">
      ...
      {!configured ? (
        ...
        <GuardCard
          icon={LogIn}
          title={t('calSub.signInTitle', { defaultValue: '登入後就可以訂閱' })}
        >
          <p>
            {t('calSub.signInBody', {
              defaultValue:
          ...
          <div className="mt-4 flex justify-center">
            <Button icon={LogIn} onClick={() => void signInWithGoogle()}>
              {t('calSub.signInGoogle', { defaultValue: '用 Google 登入' })}
            </Button>
          </div>
        </GuardCard>
      ) : !webcalUrl ? (
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-accent/30 bg-accent-soft/50 p-4 dark:border-accent/30 dark:bg-accent/10">
            <p className="flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-[0.18em] text-accent-strong/80 dark:text-accent">
              ...
              {t('calSub.yourLink', { defaultValue: '你的訂閱連結' })}
            </p>
            <a
              href={webcalUrl}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              >
                {t('calSub.copy', { defaultValue: '複製連結' })}
              </Button>
              <a
                href={webcalUrl}
          ...
          <div className="space-y-3 rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-slate-700/60 dark:bg-slate-800/40">
            ...
            <Steps
              items={[
                ...
                t('calSub.step2', {
                  defaultValue:
                    '或手動：開「設定」→「日曆」→「帳戶」→「加入帳戶」→「加入已訂閱的日曆」，貼上連結。',
                }),
                t('calSub.step3', {
                  defaultValue:
          ...
          <div className="space-y-3 border-t border-slate-200/70 pt-4 dark:border-slate-700/60">
            ...
            <div className="flex justify-end">
              <Button variant="ghost" icon={RefreshCw} onClick={handleRotate}>
                {t('calSub.regenerate', { defaultValue: '重新產生連結' })}
              </Button>
            </div>
          </div>
        </div>
