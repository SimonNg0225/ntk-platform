...
//  鍵盤快捷鍵速查表 — 純資料 + 純篩選邏輯
//  ------------------------------------------------------------
//  成個 app 散落了很多鍵盤快捷（⌘K 指令面板、全域搜尋、Inbox、資源庫、
//  Flashcards / Focus / Quiz 評分鍵…）。這裡集中成「靜態真相」的資料表，
//  給全域速查 Modal（按 ? 彈出）顯示。UI 不放這裡（保持純函式可測）。
//
//  每個快捷以 keys[]（一條一條 <Kbd>）+ desc（繁中說明）表示；
//  keys 之間的顯示分隔（例如 "或" / 連寫）交給 UI，這裡只資料。
// ============================================================
...
// ── 全部快捷（按區域分組）──────────────────────────────────
// 來源：實際 code 內的 keydown 處理（App.tsx / GlobalSearch / Inbox /
...
export const SHORTCUT_SECTIONS: ShortcutSection[] = [
  {
    ...
    items: [
      ...
      { keys: ['⌘', 'J'], desc: '開／關快速記低（自然語言 → 待辦／提醒／行事曆）' },
      { keys: ['?'], desc: '彈出這個快捷鍵速查（Shift + /）' },
  ...
  },
  {
    ...
    items: [
      ...
      { keys: ['↓'], desc: '下一個結果' },
      { keys: ['↵'], desc: '開啟選中結果' },
      { keys: ['⌘', '↵'], desc: '次要動作（例如在新分頁開）' },
      { keys: ['⌘', '1–9'], desc: '快速跳去第 N 個結果' },
      { keys: ['Tab'], desc: '切換結果類別（Shift + Tab 反向切換）' },
  ...
  },
  {
    ...
    items: [
      { keys: ['←'], desc: '上一題' },
      { keys: ['→'], desc: '下一題' },
      { keys: ['A–Z'], desc: '選擇選擇題對應選項' },
      { keys: ['1–9'], desc: '選擇第 N 個選項' },
      { keys: ['↵'], desc: '提交 / 落一題' },
      { keys: ['F'], desc: '標記此題' },
    ],
  },
...
/** 一條快捷可供搜尋的文字（說明 + 全部按鍵），統一細楷 */
function haystack(s: Shortcut): string {
  return (s.desc + ' ' + s.keys.join(' ')).toLowerCase()
}

/**
 * 按查詢字篩選快捷區段：保留「標題 / scope 命中」的整個區段，
 * 否則只保留命中的 item；篩走變空的區段。
 * - 空白查詢：原樣回傳（包括區段同 item 次序）。
 * - 大細楷不敏感、頭尾空白略過。
 * - 純函式：不 mutate 入參、同樣輸入永遠同樣輸出。
 */
