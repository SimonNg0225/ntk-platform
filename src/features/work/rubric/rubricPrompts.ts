...
function subjLine(subjectName?: string): string {
  return subjectName ? `你是香港${subjectName}科老師。` : '你是香港老師。'
}

// ───────── 評分指引 ─────────
export function buildSchemeSystem(subjectName: string | undefined, totalMarks: number): string {
  return [
    `${subjLine(subjectName)}為老師提供的題目擬一份評分指引（marking scheme），總分 ${totalMarks} 分。`,
...
export function buildRubricSystem(subjectName: string | undefined, levelCount: number): string {
  return [
    `${subjLine(subjectName)}為老師提供的題目／任務擬一份分析式評分量表（analytic rubric），每個準則 ${levelCount} 個表現等級（由高到低）。`,
    '只輸出一個 JSON 物件，不要有任何其他文字或 markdown code fence：',
    '{',
...
export function parseRubric(raw: string): Rubric {
  ...
  const criteria: RubricCriterion[] = Array.isArray(o.criteria)
    ...
    : []
  if (criteria.length === 0) throw new Error('AI 出不到評分量表，嘗試換 Pro 或補充題目。')
  return { criteria }
}
