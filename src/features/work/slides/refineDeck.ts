...
/** 結構潤飾指引（聚焦結構問題，不重抄 slidePrompts 全本 playbook）。 */
const REFINE_PLAYBOOK: string[] = [
  '結論先行：弱標題（「XX 簡介／概述」式）改成有觀點、有資訊量的標題。',
  '太逼就拆：一版超過 5 點，拆做兩版，或升做 stats / steps / cards 等版式。',
  '版式要有節奏：避免連續 3 版都是 bullets — 適度改用 stats（數據）、compare（對比）、steps（流程）、cards（並列）、quote（金句）。',
  '輕重分明：全套選擇 1-2 版最關鍵的標 emphasis:true（高潮版），其餘不要亂標。',
  '關鍵頁補 takeaway：一句 ≤46 字的總結重點，協助聽眾記住。',
  '每點精煉 ≤60 字、每版 ≤6 點；stats 2-4 項、steps 2-5 步、cards 2-6 張、compare 每邊 2-4 點。',
]

/** 由 pack 帶入版式偏好（輕量，不重抄 slidePrompts 的 PACK_FAVORS）。 */
...
export function buildRefineSystem(subjectName?: string, pack?: SlidePackId): string {
  ...
  return [
    '你是資深教學簡報設計師。輸入是一套完整簡報的 JSON（schema 與生成時一致）。',
    `${subj}你的工作：只做結構同文案優化，令簡報更清晰、更有節奏。`,
    '',
    '潤飾指引：',
    ...
    '',
    '硬規則：',
    '· 不可以作新事實／新數據（只可重組、收緊文字、調版式）。',
    '· 不可以改 deck.title 的主題。',
    '· 版數可微調，但不要大幅增減（保持原本規模）。',
    '· 輸出必須係同 schema 的完整 deck JSON：{title, subtitle?, coverImageQuery?, slides:[...]}。',
    '· 只輸出 JSON，不要有任何前後文字、不要用 markdown fence。',
  ].join('\n')
}

/**
 * 跑一次結構潤飾 pass。網路在 complete、校驗在 parseDeck（throw 就照拋給 UI）。
 * 預設用 flash（structure pass 不需要 pro）。
 */
 ...
 * coverImageQuery 若 refined 沒有出就保留 original；slides 大幅縮水（<50%）就拒絕、保留原樣。
 * 純 logic（vitest 重點）。
 */
export function mergeRefinedDeck(original: Deck, refined: Deck): Deck {
  // 縮水防呆：refined 版數少過原本一半 → 不採納 slides（其餘照原樣）
