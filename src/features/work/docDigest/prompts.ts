...
/** 組裝 system prompt。today = 今日 YYYY-MM-DD，用來換算相對日期。 */
export function buildDigestSystem(today: string): string {
  return [
    '你是香港學校的行政文件助手。使用者會提供一份行政／校務文件（通告、會議文件、表格、政策、家長信等，可能是文字或圖片）。',
    '請快速「速讀」，然後**只輸出一個 JSON 物件**，不要有任何其他文字、解釋或 markdown code fence。',
    '格式：',
    '{',
    '  "title": "一句精簡標題（≤20字）",',
    `  "category": "由以下其中一個選擇最貼切：${DIGEST_CATEGORIES.join(' / ')}",`,
    '  "summary": ["3至6個重點，每點一句精簡繁體中文"],',
    '  "actions": [{"text": "老師要跟進的事項（動作＋對象）", "date": "YYYY-MM-DD（無明確截止日就省略）"}]',
    '}',
    '規則：',
    '- 一律用繁體中文（可書面廣東話）。',
    '- summary 抽最重要的資訊：決定、時間、地點、要求、金額等。',
    '- actions 只放「老師要做」的跟進事項；沒有就用空陣列 []。',
    `- 日期換算成實際 YYYY-MM-DD（今日係 ${today}）。相對日期（如「下星期五」）盡量推算；不肯定就不要作 date，直接省略。`,
    '- 一定要係有效 JSON，不可以有多餘文字。',
  ].join('\n')
}
...
export function parseDigest(raw: string): DigestResult {
  ...
  if (!json) throw new Error('AI 回應格式不正確，請再試一次。')
...
/** 由可能夾雜 code fence / 前後文字的字串，抽出第一個 {...} JSON 區塊。 */
