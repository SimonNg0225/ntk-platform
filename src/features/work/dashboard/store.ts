...
// ============================================================
//  工作儀表板：功能專屬持久化（不掂 data/collections.ts）
//  ------------------------------------------------------------
//  儀表板的資料全部係跨功能彙整返來（tasks / timetable / events…），
//  本身唯一需要持久化的係「版面設定」：
//    - 邊些 widget 顯示 / 隱藏
//    - widget 排列次序
//    - 統計時間範圍（7 / 14 / 30 日）
//  以「單一 row」存在自己的 collection（id 固定 'layout'）。
//  唯一 key（已在 newCollections 申報）：work_dashboard_layout
// ============================================================

export interface DashboardLayout {
  id: string // 固定 'layout'
  order: WidgetId[] // widget 排列次序（同時決定顯示者）
  hidden: WidgetId[] // 明確收起的 widget
...
export function readLayout(): DashboardLayout {
  ...
  // 將「新加入但 saved order 未有」的 widget 補去尾，確保升級不會漏 widget
...
function commit(patch: Partial<Omit<DashboardLayout, 'id'>>) {
  // readLayout 必定回傳一個 row（無就會 add 預設），亦會補上升級後新增的 widget。
  // 用 set 整批覆寫（順帶把 migration 的 order 落地），避免 update 漏了 migrated 欄位。
