...
// 六準則固定次序 + 廣東話 label（UI 同 prompt 共用這個 single source of truth）
...
export function parseObservationSummary(raw: string): ObservationSummary {
  ...
  // 全空（六準則 note 皆空 + 沒有亮點 + 沒有建議）= AI 實質上沒有撮要到嘢，
  // 視為失敗 throw，行返 retry 流程，不要落地一個全空的死記錄。
  const hasAnything =
    criteria.some((c) => c.note.trim()) || highlights.length > 0 || improvements.length > 0
  if (!hasAnything) throw new Error('AI 今次抽不到觀察內容，請再試一次。')
