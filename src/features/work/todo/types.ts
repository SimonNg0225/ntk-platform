...
//  ------------------------------------------------------------
//  共用 tasksCol（Task: text / done / createdAt）維持不變，
//  係單一真相來源。本功能需要、而 Task 沒有的屬性（優先級 /
//  到期 / 專案 / 標籤 / 備註 / 排序）+ 子任務 + 專案清單，
//  全部存在這個 feature 自己的 collection（不掂 data/collections.ts）。
//  TaskMeta 以「task.id」做 id（一對一 sidecar）。
// ============================================================

// P1 最重要 → P4 無（Todoist 慣例）
export type Priority = 1 | 2 | 3 | 4

// 一條任務的延伸中繼資料（sidecar，key = task.id）
...
// 專案（清單；Things 的「Project」/ Todoist 的「Project」）
