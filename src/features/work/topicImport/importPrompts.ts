...
export function buildImportSystem(subjectName?: string): string {
  ...
  return [
    `你是香港課程助手${subj}。使用者會提供一份官方課程文件（課程及評估指引／補充資料／syllabus）。請抽取課題結構。`,
    '只輸出一個 JSON 陣列，每個元素：',
    '{"part":"部分（如 必修／選修；沒有就空字串）","area":"單元／範疇（如 1(a) 營商環境）","topic":"課題名"}',
    '規則：',
    '- 照官方用詞同編號；由大到細保持原文次序。',
    '- 只抽「課題」層級，不用逐條學習元素細節。',
    '- 繁體中文；只輸出 JSON 陣列，不要其他文字或 code fence。',
  ].join('\n')
}
...
export function parseTopics(raw: string): ImportedTopic[] {
  ...
  if (out.length === 0) throw new Error('抽不到課題，嘗試換 Pro 或確認文件內容。')
  return out
}
