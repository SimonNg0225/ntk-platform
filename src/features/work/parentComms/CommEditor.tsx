...
const REMIND_OPTIONS: { value: number; key: string; zh: string }[] = [
  { value: 0, key: "parent.remindSameDay", zh: "當日" },
  { value: 60 * 24, key: "parent.remind1day", zh: "提前 1 日" },
  { value: 60 * 24 * 2, key: "parent.remind2days", zh: "提前 2 日" },
  { value: 60 * 24 * 7, key: "parent.remind1week", zh: "提前 1 週" },
];
...
export default function CommEditor({
  ...
  const applyTemplate = (tpl: CommTemplate) => {
    setD((prev) => ({
      ...prev,
      channel: (CHANNELS as readonly string[]).includes(tpl.channel)
        ? (tpl.channel as Channel)
        : prev.channel,
    ...
    <Modal open={open} onClose={onClose} size="lg">
      ...
      <header className="relative -mx-5 -mt-5 overflow-hidden px-5 pb-4 pt-5 sm:-mx-6 sm:-mt-6 sm:px-6 sm:pt-6">
        ...
        >
          ...
          {editing
            ? t("parent.editorStampEdit", { defaultValue: "修函" })
            : t("parent.editorStampNew", { defaultValue: "草擬" })}
        </span>
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h2 className="mt-1.5 font-serif text-[24px] font-semibold leading-none tracking-tight text-slate-800 dark:text-slate-100 sm:text-[26px]">
              {editing
                ? t("parent.editorTitleEdit", { defaultValue: "編輯信件" })
                : t("parent.editorTitleNew", { defaultValue: "撰寫信件" })}
            </h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label={t("parent.close", { defaultValue: "關閉" })}
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <section className="space-y-3">
          <SectionLabel icon={Users}>
            {t("parent.recipientSection", { defaultValue: "收件人" })} ·
            Recipient
          </SectionLabel>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field
              label={t("parent.classRequired", { defaultValue: "班別" })}
              required
            >
              <Select
                value={d.classId}
              ...
              >
                <option value="">
                  {t("parent.pickClass", { defaultValue: "請揀班別" })}
                </option>
                {classes.map((c) => (
            ...
            <Field
              label={t("parent.studentOptional", {
                defaultValue: "學生（選填）",
              })}
            >
              <Select
              ...
              >
                <option value="">
                  {d.classId
                    ? t("parent.wholeClass", { defaultValue: "全班 / 不指定" })
                    : t("parent.pickClassFirst", {
                        defaultValue: "請先揀班別",
                      })}
                </option>
                {formStudents.map((s) => (
        ...
        <section className="space-y-3">
          <SectionLabel icon={CalendarClock}>
            {t("parent.dispatchSection", { defaultValue: "寄件詳情" })} ·
            Dispatch
          </SectionLabel>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field
              label={t("parent.letterDirection", { defaultValue: "信件方向" })}
            >
                ...
                {DIRECTIONS.map((dir) => {
                  ...
                  return (
                    ...
                    >
                      <DirIco size={14} />
                      {incoming
                        ? t("parent.direction.incoming", {
                            defaultValue: "來函",
                          })
                        : t("parent.direction.outgoing", {
                            defaultValue: "發函",
                          })}
                    </button>
                  );
            ...
            <Field
              label={t("parent.contactNameLabel", {
                defaultValue: "收信人稱呼（選填）",
              })}
              hint={t("parent.contactNameHint", {
                defaultValue: "例如：陳太、父親",
              })}
            >
              <Input
                value={d.contactName}
                onChange={(e) => set("contactName", e.target.value)}
                placeholder={t("parent.contactNamePlaceholder", {
                  defaultValue: "家長 / 監護人稱呼",
                })}
              />
            </Field>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <Field label={t("parent.dateLabel", { defaultValue: "日期" })}>
              <Input
                type="date"
                value={d.date}
            ...
            <Field
              label={t("parent.channelLabel", { defaultValue: "聯絡方式" })}
            >
              <Select
                value={d.channel}
            ...
            <Field
              label={t("parent.categoryLabel", { defaultValue: "主題分類" })}
            >
              <Select
                value={d.category}
                onChange={(e) =>
              ...
              >
                <option value="">
                  {t("parent.categoryUnset", { defaultValue: "未分類" })}
                </option>
                {CATEGORY_OPTIONS.map((o) => (
                  <option key={o.value} value={o.value}>
                    {t(`parent.category.${o.value}`, { defaultValue: o.label })}
                  </option>
                ))}
              </Select>
            </Field>
        ...
        <section className="space-y-2.5">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              ...
              {t("parent.letterBody", { defaultValue: "信文" })} · Letter body
              ...
              <Menu
                ...
                trigger={
                  <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-medium text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
                    <Sparkles size={13} />
                    {t("parent.useTemplate", { defaultValue: "取信件範本" })}
                  </span>
                }
          ...
          <div className="overflow-hidden rounded-xl ring-1 ring-slate-200/80 transition focus-within:ring-2 focus-within:ring-accent/40 dark:ring-slate-700/70">
            <span
              aria-hidden
              className="block h-0.5 bg-accent/30 dark:bg-accent/25"
            />
            <Textarea
              id="comm-body"
              ...
              placeholder={t("parent.bodyPlaceholder", {
                defaultValue: "敬啟者…\n記低溝通內容、家長回應、達成共識。",
              })}
            />
          </div>
          <p className="px-0.5 text-xs text-slate-400 dark:text-slate-500">
            {t("parent.bodyHint", {
              defaultValue: "落筆記低呢次往來嘅重點，日後喺通訊錄一翻就見到。",
            })}
          </p>
        </section>
        ...
        {/* 觀感 */}
        <Field
          label={t("parent.outcomeSection", {
            defaultValue: "家長回響（選填）",
          })}
        >
        ...
        >
          <label className="flex cursor-pointer items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
            ...
            {t("parent.followToggle", {
              defaultValue: "待回覆跟進（記低回郵到期）",
            })}
          </label>
          {d.followUp && (
            <div className="mt-3 space-y-3">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <Field
                  label={t("parent.remindLabel", { defaultValue: "提醒" })}
                >
                  <Select
                    value={
                      d.remindMinutes === "" ? "" : String(d.remindMinutes)
                    }
                  ...
                  >
                    <option value="">
                      {t("parent.remindNone", { defaultValue: "不提醒" })}
                    </option>
                    {REMIND_OPTIONS.map((o) => (
                      <option key={o.value} value={o.value}>
                        {t(o.key, { defaultValue: o.zh })}
                      </option>
                    ))}
                  </Select>
                </Field>
              ...
              <Field
                label={t("parent.followNoteLabel", {
                  defaultValue: "回覆待辦（選填）",
                })}
              >
                <Input
                  value={d.followUpNote}
                  onChange={(e) => set("followUpNote", e.target.value)}
                  placeholder={t("parent.followNotePlaceholder", {
                    defaultValue: "例如：再致電了解測驗準備情況",
                  })}
                />
              </Field>
              <div className="flex flex-wrap gap-1.5">
                {[
                  { key: "parent.presetTomorrow", zh: "明日", days: 1 },
                  { key: "parent.preset3days", zh: "3 日後", days: 3 },
                  { key: "parent.preset1week", zh: "1 週後", days: 7 },
                  { key: "parent.preset2weeks", zh: "2 週後", days: 14 },
                ].map((q) => (
                  <button
                    key={q.days}
                    type="button"
                    onClick={() =>
                      set("followUpDate", shiftKey(todayKey(), q.days))
                    }
        ...
        {/* 動作列：信箋雙線分隔，呼應封面 */}
        <div
          className="-mx-5 mt-1 space-y-1 px-5 pt-1 sm:-mx-6 sm:px-6"
          aria-hidden
        >
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="secondary" onClick={onClose}>
            {t("parent.cancel", { defaultValue: "取消" })}
          </Button>
          <Button type="submit" disabled={!canSubmit} icon={Send}>
            {editing
              ? t("parent.saveChanges", { defaultValue: "儲存變更" })
              : t("parent.saveNew", { defaultValue: "存入通訊錄" })}
          </Button>
        </div>
      </form>
    </Modal>
