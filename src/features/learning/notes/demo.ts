...
//  Notes 示範資料 seeder
//  ------------------------------------------------------------
//  一鍵填入真實感、連貫的樣本筆記，令空白 app 立即「有嘢查看」。
//  只負責 richNotesCol（筆記）。notebooksCol 已有內建種子（DSE 溫習 /
//  靈感速記），這裡的筆記直接掛返嗰兩個現有筆記本 id。
//
//  Idempotent：richNotesCol 現在係空（.get().length === 0）先種，
//  已有資料就跳過，不會重複塞。回傳實際新增的 row 數。
// ============================================================

// 本地日期 helper：以「當地中午」砌返第 n 日前的 Date，
// 轉 ISO 後 createdAt/updatedAt 會落在預期那個本地日子，
// 避開 toISOString 直接用 0 點時的 UTC 時差漂移（與 store/util
// 內 dayKey 以本地年月日分桶的做法一致）。
...
// 已存在的內建筆記本 id（見 store.ts notebooksCol 種子）
const NB_DSE = 'nb-dse'
...
interface NoteSeed {
  ...
  favorite?: boolean
  color?: string
  // 相對「今日」的天數：created 較舊、updated 較近（updated >= created）
