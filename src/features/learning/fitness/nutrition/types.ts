...
//  AI 飲食營養 — 本模組專屬型別
//  ------------------------------------------------------------
//  共用 ui / store / common 不可改。凡係本模組獨有的資料
//  （單筆飲食、每日目標、AI 解析出來的食物項）一律在這裡定義。
//  日期一律用 ../common 的本地 key（YYYY-MM-DD，避 UTC 漂移）。
// ============================================================

/**
 * 一餐分段標籤。舊資料無 `meal` 一律當 'other'（向後相容）。
 * 順序即係日誌分組顯示的次序（早→午→晚→小吃→其他）。
 */
...
/**
 * AI 由自然語言解析出來的食物項（未落地，純前端草稿）。
 * 同 FoodEntry 的四個營養欄位對齊，方便逐項微調再存。
 * 每項配一個前端臨時 key，給編輯 / 移除定位用。
 */
...
/** AI 回應的原始 shape（未必齊 / 未必數字，逐欄 runtime 收斂） */
...
/**
 * 「常吃」統計結果（純前端、由歷史 FoodEntry 去重聚合，不落地）。
 * 同名 + 同 macros 視為同一款，count 為出現次數，一按即可再加回今日。
 */
