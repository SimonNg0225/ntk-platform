...
//  體態數據 — 示範資料 seeder
//  ------------------------------------------------------------
//  一鍵填入一個「有上進心、生活忙碌」的人近 4 週的 InBody 式身體
//  組成記錄：體重穩步輕微落、體脂率慢慢跌、骨骼肌量略升 —— 即係理想
//  的「體態重組（recomp）」走勢，令趨勢圖 / KPI / 增肌減脂分析 /
//  目標進度條一開即有嘢查看。配身高 + 目標 + 內容來源體重 profile。
//
//  鐵則：
//   - idempotent —— 每個 collection 只在他現在係空（length === 0）先種。
//   - 日期一律行健身共用的本地日期 helper（../common 的 recentDays），
//     分佈在最近約 4 週，不用未來日。
//   - 真實感：不是日日做完整 InBody —— 個別日子淨秤體重（缺體脂 / 肌肉），
//     量度有輕微上落噪聲，不是一條直線。
//   - 純資料，不掂 UI / 不 import React。
// ============================================================

/** 一個量度日的指標（缺值 = 嗰日沒有量該項）。offset = 距今日數（愈大愈舊）。 */
interface DayPlan {
  offset: number
  weightKg?: number
  bodyFatPct?: number
...
export function seedDemo(): number {
  ...
  // ── 1. 體態設定（單例）：身高 + 目標 + 內容來源 ────────────────
  // 有齊先計到 BMI / 目標進度條 / 達標預計。內容來源對齊 PLAN 最舊一筆體重。
  ...
  // ── 2. 每日身體組成記錄（近 4 週）────────────────────────
  // recentDays(28) 由舊到新；用 offset 對應「距今第幾日」取得 key。
