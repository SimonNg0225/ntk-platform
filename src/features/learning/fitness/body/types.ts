...
/** 可量度數值指標 key（給趨勢 / KPI 共用）。內臟脂肪係等級，分開處理。 */
export type MetricKey = 'weightKg' | 'bodyFatPct' | 'skeletalMuscleKg'

/** 體態設定（單例）：身高用來計 BMI；體重目標用來計進度 / 達標預計。 */
export interface BodyProfile extends Entity {
  heightCm?: number
  /** 目標體重（kg）。可選；沒有就不顯示進度條。 */
  weightTargetKg?: number
  /**
   * 進度內容來源體重（kg）。可選 —— 舊資料沒有此欄，UI 會 fallback 用最早一筆
   * 體重記錄做內容來源，保持向後相容。
   */
