...
//  知識卡（Anki 級）— 本功能專屬型別
//  ------------------------------------------------------------
//  共用 data/types 的 Card / Deck 不可改，所以凡係要「加落卡」的
//  額外屬性（標籤、暫停、leech…）一律存在本功能自家 collection，
//  以 cardId 關聯。複習歷史亦然（heatmap / 留存率 / 預測都靠他）。
// ============================================================

/** 每張卡的額外中繼資料（以 cardId 關聯共用 Card） */
export interface CardMeta extends Entity {
  // id === cardId（直接用卡 id 做 key，不用另一層對照）
  tags: string[]
  suspended: boolean // 暫停：不會出現在複習隊列
  flagged: boolean // 標記（紅旗）方便篩
  lapses: number // 累計「不記得」次數（用來判 leech）
  note?: string // 私人備註（額外提示）
  updatedAt: string
}

/** 一次複習的 log（每答一張卡寫一條，係統計核心） */
export interface ReviewLog extends Entity {
  cardId: string
  deckId: string
  ts: string // ISO，答題一刻
  rating: Rating // again / hard / good / easy
  prevInterval: number // 答之前的 intervalDays
  newInterval: number // 答之後的 intervalDays
  elapsedMs: number // 由翻面到評分用了幾耐（思考 + 回答）
  mode: StudyMode // 在邊種模式複習
}
