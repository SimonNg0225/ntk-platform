...
//  同學習儀表板都由這裡 import，確保寫入即時通知對方（避免重複
//  createCollection 同 key 而各自一份 in-memory store 不同步）。
//  放在獨立 store 檔（對齊 focus/store、notes/store），等 journal/util
//  保持零副作用、純函式。首次由舊 journal 遷移在 Journal.tsx 處理。
// ============================================================
