...
兩個 [Scriptable](https://scriptable.app/)（免費 iOS app）腳本，喺 iPhone 主畫面顯示你嘅 EziTeach AI 數據：
...
EziTeach AI 係網頁 / PWA。iOS 主畫面 widget **一定要原生 WidgetKit**，網頁攞唔到。
...
2. Scriptable → ＋ → 改名 `EziTeach AI` → 將 [`ntk-widget.js`](./ntk-widget.js) 全文貼入去。
3. 填好檔頂 `CONFIG`：
...
4. 主畫面長按 → ＋ → Scriptable → 中型 widget → 編輯 → Script 揀 `EziTeach AI`。

**今日課堂 widget**：重覆步驟 2–4，但貼 [`ntk-timetable-widget.js`](./ntk-timetable-widget.js)、改名 `EziTeach 課堂`、揀**中 / 大型** widget。
